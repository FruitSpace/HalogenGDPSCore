	...
	switch($command[0]){
		...
		case "!coins":
			...
			if($command[1]=="verify"){
				...
				err_handle("modCP","err","!coins were not verify or resett and avoided filters: ".json_encode($command[1]));
			}
			return 1;
		case "!daily":
		...
		case "!lvl":
			...
			switch ($command[1]){
				...
				case "desc":
					...
					$dbm->preparedQuery("UPDATE levels SET description=? WHERE id=$lvl_id","s",str_replace("!lvl desc ","",$comment));
					return 1;
				case "list":
					if(!$isOwner and $privs['cLvlAccess']!=1) return -1;
